...
function runLabel(r: PayrollRunInfo): { text: string; color: string } {
      ...
      {loading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center gap-3 py-12 text-center">
            ...
            <p className="max-w-xs text-xs text-gray-600">Create your first pay run to start paying employees.</p>
            <Link to={`/employer/${payrollAddr}/payroll/new`}>
              <Button><Plus className="mr-2 h-4 w-4" /> New Pay Run</Button>
            </Link>
          </CardContent>
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-gray-500 mt-0.5">
                          {date}
                          {r.status === 0 && r.batchProcessed > 0 && <span className="ml-2 text-orange-400">({r.batchProcessed}/{r.employeeCount} processed)</span>}
                        </p>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
